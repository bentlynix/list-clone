...
test("drag event gets emitted when clicking an item", function (t) {
  ...
  testCase(
    ...
    { which: 1 },
    { passes: false, dragulaOpts: { invalid: always } }
  );
  testCase(
    "fails whenever moves returns false",
    { which: 1 },
    { passes: false, dragulaOpts: { moves: never } }
  );
  t.end();
  function testCase(desc, eventOptions, options) {
    t.test(desc, function subtest(st) {
      ...
      function drag(target, container) {
        st[passes ? "pass" : "fail"](
          desc + ": drag event was emitted synchronously"
        );
...
test("when copying, emits cloned with the copy", function (t) {
  ...
  function cloned(copy, item) {
    ...
    t.equal(
      copy.tagName,
      item2.tagName,
      "first argument has same tag as target"
    );
    t.equal(
      copy.innerHTML,
      item2.innerHTML,
      "first argument has same inner html as target"
    );
...
test("when dragging, body gets gu-unselectable class", function (t) {
  ...
  t.equal(
    ...
    "gu-unselectable",
    "body has gu-unselectable class"
  );
  t.end();
});
...
test("when dragging, element gets a mirror image for show", function (t) {
  ...
  function cloned(mirror, target) {
    ...
    t.equal(
      mirror.innerHTML,
      item.innerHTML,
      "mirror is passed to 'cloned' event"
    );
    t.equal(
      target,
      item,
      "cloned lets you know that the mirror is a clone of `item`"
    );
  }
});
...
test("when dragging, mirror element gets appended to configured mirrorContainer", function (t) {
  ...
  function cloned(mirror) {
    t.equal(
      mirror.parentNode,
      mirrorContainer,
      "mirrors parent is the configured mirrorContainer"
    );
  }
});
...
test("when dragging stops, body becomes selectable again", function (t) {
  ...
  t.equal(
    document.body.className,
    "",
    "body got gu-unselectable class removed"
  );
  t.end();
});
...
test("when drag begins, check for copy option", function (t) {
  ...
  function checkCondition(el, source) {
    ...
    t.equal(
      source.className,
      "contains",
      "source container classname is contains"
    );
    return true;
  }
  drake.end();
