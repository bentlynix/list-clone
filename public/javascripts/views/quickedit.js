var QuickEditView = Backbone.View.extend({
  ...
  changePositionOptions: function (e) {
    var selectedListNameId = +$("#lists_of_lists option:selected").attr(
      "data-id"
    );
  ...
  showCopyPopup: function (e) {
    ...
    App.listNames.forEach(function (obj) {
      $("#lists_of_lists_for_copy").append(
        $("<option></option>")
          .attr("value", obj.name)
          .text(obj.name)
          .attr("data-id", obj.id)
    ...
    for (var i = 1; i <= length + 1; i++) {
      $("#card_positions_for_copy").append(
        $("<option></option>").attr("value", i).text(i)
      );
    }
  ...
  changeCopyPositionOptions: function (e) {
    var selectedListNameId = +$(
      "#lists_of_lists_for_copy option:selected"
    ).attr("data-id");
    var selectedListLength = App.lists.cardCounts.filter(function (list) {
      return list.id === selectedListNameId;
    })[0].length;
  ...
  copyCard: function (e) {
    var targetListId = +$("#lists_of_lists_for_copy option:selected").attr(
      "data-id"
    );
