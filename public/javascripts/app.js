var App = {
  ...
  setCardsCountInList: function () {
    ...
    this.lists.forEach(function (list) {
      ...
      var cardsInCurrentList =
        App.cards.where({ listId: currentListId }).length || 0;
  ...
  addCard: function (id, content) {
    ...
    $.ajax({
      ...
      data: {
        ...
        name: content,
        activity:
          "Grunt L. added this card to list " +
          currentList.get("name") +
          " " +
      ...
      success: function (json) {
        ...
        if (currentList.get("subscribed")) {
          App.notifications.unshift({
            cardId: json.id,
            detail:
              "Grunt L. added a new card to list: " +
              currentList.get("name") +
              " " +
  ...
  toggleSubscription: function (cardId) {
    this.currentCard.set(
      "subscribed",
      this.currentCard.get("subscribed") ? false : true
    );
      ...
      success: function () {
        ...
        if (currentList.get("subscribed")) {
          App.notifications.unshift({
            cardId: null,
            detail:
              "Grunt L. archived all cards from list: " +
              currentList.get("name") +
              " " +
              App.formatDate(new Date()),
          });
      ...
      success: function () {
        ...
        if (App.currentCard.get("subscribed")) {
          App.notifications.unshift({
            ...
            detail:
              "Grunt L. commented on card " +
              App.currentCard.get("name") +
              " " +
              App.formatDate(new Date()),
          });
  ...
  updateCardDescription: function (description) {
    ...
    this.currentCard.get("activity").push({
      activity:
        "Grunt L. updated this card's description " +
        this.formatDate(new Date()),
    });
      ...
      success: function () {
        ...
        if (App.currentCard.get("subscribed")) {
          App.notifications.unshift({
            cardId: App.currentCard.get("id"),
            detail:
              "Grunt L. updated the description of card: " +
              App.currentCard.get("name") +
              " " +
              App.formatDate(new Date()),
          });
  ...
  updateLabel: function (newLabel) {
    ...
    if (existingLabels.indexOf(newLabel) === -1) {
      ...
      this.currentCard.get("activity").push({
        activity: "Grunt L. added a label " + this.formatDate(new Date()),
      });
    } else {
      this.currentCard.set(
        "labels",
        this.currentCard.get("labels").filter(function (label) {
          return label.color !== newLabel;
        })
      );
      this.currentCard.get("activity").push({
        activity: "Grunt L. removed a label " + this.formatDate(new Date()),
      });
    }
      ...
      success: function () {
        ...
        if (App.currentCard.get("subscribed")) {
          App.notifications.unshift({
            cardId: App.currentCard.get("id"),
            detail:
              "Grunt L. updated a label on card: " +
              App.currentCard.get("name") +
              " " +
              App.formatDate(new Date()),
          });
  ...
  editCardName: function (newName) {
    ...
    this.currentCard.get("activity").push({
      activity:
        "Grunt L. edited the name of this card from " +
        '"' +
        oldName +
        '"' +
        " to " +
      ...
      success: function () {
        ...
        if (App.currentCard.get("subscribed")) {
          App.notifications.unshift({
            cardId: App.currentCard.get("id"),
            detail:
              "Grunt L. changed the name on card: " +
              oldName +
              " " +
              " to " +
              App.currentCard.get("name") +
  ...
  setDueDate: function (date) {
    ...
    if (date) {
      ...
      this.currentCard.get("activity").push({
        activity: "Grunt L. removed due date " + this.formatDate(new Date()),
      });
    }
  ...
  moveSingleCard: function (targetListId, targetPosition, cardId) {
    ...
    var cardsInTargetList = this.cards.where({ listId: targetList.get("id") })
      .length;
    ...
    if (targetPosition > cardsInTargetList) {
      ...
      card.set("cardPosition", targetPosition + 0.5);
    } else {
      card.set("cardPosition", targetPosition - 0.5);
    }
      ...
      success: function () {
        ...
        if (targetListSubscribed || currentListSubscribed) {
          App.notifications.unshift({
            cardId: null,
            detail:
              "Grunt L. moved all cards from list: " +
              currentListName +
              " to list " +
              targetListName +
              " to " +
  ...
  copyCard: function (targetListId, targetPosition, cardName, cardId) {
    ...
    if (targetPosition >= cardsInTargetList) {
      ...
      copiedCard.set("cardPosition", targetPosition - 0.5);
    }
  ...
  moveList: function (position, listId) {
    ...
    if (position === 1) {
      ...
      currentList.set("position", position - 0.5);
    } else {
      currentList.set("position", position + 1);
    }
    this.lists.sort();
  ...
  closeModal: function (e) {
    ...
    if (this.currentCard.view) {
      if (
        $("#label_popup:visible").length > 0 ||
        $("#date_popup:visible").length > 0 ||
        $("#move_card_popup:visible").length > 0 ||
        $("#copy_card_popup:visible").length > 0
      ) {
